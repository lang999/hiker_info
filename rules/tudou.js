const csdown = {
    ...
    movie_new: function() {
        ...
        try {
            ...
            d.push({
                ...
                url: $('#noLoading#').lazyRule(() => {
                    ...
                    let Line = {
                        title: '切换年份',
                        options: year,
                        selectedIndex: year.indexOf(getMyVar('movie_year', getMyVar('movie_year_index'))),
                        col: 1,
                        js: $.toString(() => {
                ...
                url: $('#noLoading#').lazyRule(() => {
                    ...
                    let Line = {
                        title: '切换月份',
                        options: month,
                        selectedIndex: month.indexOf(getMyVar('movie_month', getMyVar('movie_month_index'))),
                        col: 2,
                        js: $.toString(() => {
                ...
                url: $('#noLoading#').lazyRule(() => {
                    ...
                    let day = getDaysInMonth(getMyVar('movie_year', getMyVar('movie_year_index')), getMyVar('movie_month', getMyVar('movie_month_index')));
                    let Line = {
                        title: '切换日期',
                        options: day,
                        selectedIndex: day.indexOf(getMyVar('movie_day', getMyVar('movie_day_index'))),
                        col: 3,
                        js: $.toString(() => {
            ...
            let tuijian = this.post('movie/tuijian/1/10/time', JSON.stringify({
                "creattime": getMyVar('movie_year', getMyVar('movie_year_index')) + '-' + getMyVar('movie_month', getMyVar('movie_month_index')) + '-' + getMyVar('movie_day', getMyVar('movie_day_index'))
            })).list;
                ...
                d.push({
                    ...
                    extra: {
                        ...
                        longClick: [{
                            ...
                            js: $.toString(() => {
                                return $('hiker://empty?page=fypage&#immersiveTheme#&#noHistory#').rule(() => {
    ...
    post: function(url, body) {
        body = body || '';
        let data = JSON.parse(fetch(getItem('host') + url, {
            headers: {
                'token': getItem('token'),
            },
            body: body,
    ...
    comic: function() {
        ...
        try {
            if (MY_PAGE == 1) {
                ...
                fenlei.forEach(data => {
                    d.push({
                        title: comic_ == data.id ? this.strong(data.name, 'ff6699') : data.name,
                        url: $('#noLoading#').lazyRule((n, name, id) => {
                            putMyVar(n, id);
                            refreshPage(false);
                            return 'hiker://empty';
                        }, 'comic', data.name, data.id),
    ...
    comic_more: function() {
        ...
        try {
            ...
            let more = this.post('manhua/moredata/' + id + '/' + pg + '/15/time').records;
            more.forEach(data => {
                d.push({
                    title: data.title,
                    desc: '共' + data.zhangjieNum + '话',
    ...
    comic_detail: function() {
        ...
        try {
            ...
            d.push({
                title: manhua.title,
                desc: '共' + manhua.zhangjieNum + '话 ' + manhua.creatTime + '\n' + JSON.parse(manhua.marks).join(' '),
    ...
    comics: function(id) {
        return $().lazyRule((id) => {
            ...
            let pics = data.imgurl.split(',').map(data => this.$.require('csdown').images(data)).join('&&');
            return 'pics://' + pics;
        }, id)
    },
    comic_new: function() {
    ...
    book: function() {
        ...
        try {
            if (MY_PAGE == 1) {
                ...
                fenlei.forEach(data => {
                    d.push({
                        title: book_ == data.id ? this.strong(data.name, 'ff6699') : data.name,
                        url: $('#noLoading#').lazyRule((n, name, id) => {
                            putMyVar(n, id);
                            refreshPage(false);
                            return 'hiker://empty';
                        }, 'book', data.name, data.id),
                        col_type: 'scroll_button',
                        extra: {
    ...
    book_detail: function() {
        ...
        try {
            ...
            d.push({
                title: book.title,
                desc: '共' + book.zhangjieNum + '章 ' + book.creatTime + '\n' + JSON.parse(book.marks).join(' '),
            ...
            zhangjie.forEach(data => {
                d.push({
                    title: data.name,
                    img: this.images(book.imgurl),
                    url: $('hiker://empty?id=' + data.id + '&#readTheme#&#autoPage#').rule(() => {
    ...
    movie: function() {
        ...
        try {
            if (MY_PAGE == 1) {
                ...
                fenlei.forEach(data => {
                    d.push({
                        title: movie_ == data.id ? this.strong(data.name, 'ff6699') : data.name,
                        url: $('#noLoading#').lazyRule((n, name, id) => {
                            putMyVar(n, id);
                            refreshPage(false);
                            return 'hiker://empty';
                        }, 'movie', data.name, data.id),
                        col_type: 'scroll_button',
                        extra: {
                ...
                if (movie_ == getMyVar('movie_index') || movie_ == '13') {
                    ...
                    if (movie_ == getMyVar('movie_index')) {
                        ...
                        if (!storage0.getMyVar('movie_userList')) {
                            let userList = list.userList.map((data, index) => {
                                return {
                                    ...
                                    desc: data.jianjie,
                                    img: this.images(data.imgurl),
                                    url: $('hiker://empty?page=fypage&#immersiveTheme#&#noHistory#').rule(() => {
                                        $.require('csdown').movie_user();
                                    }),
                        ...
                        d.push({
                            ...
                            img: 'https://igtcqqindd4.icu/h5/static/img-manhua/mhhot.png',
                            url: $('#noLoading#').lazyRule(() => {
                                putMyVar('movie_flod', getMyVar('movie_flod', '0') == '0' ? '1' : '0');
                        ...
                        if (getMyVar('movie_flod', '0') == '0') {
                            storage0.getMyVar('movie_userList').slice(0, 4).forEach(data => {
                                d.push(data)
                            })
                        } else {
                    ...
                    list.movieList.forEach(data => {
                        ...
                        movie_list.forEach(item => {
                            ...
                            d.push({
                                title: movie_.title,
                                desc: movie_.creatTime.slice(0, 10) + '              ' + movie_.duration,
            ...
            let movie_ = getMyVar('movie', getMyVar('movie_index'));
            if (movie_ == '20') {
                let movie_list = this.post('movie/fengleidataShow/' + movie_ + '/' + pg + '/15/' + getMyVar('movie_cate', 'zx')).movieList;
                movie_list.forEach(item => {
                    let movie = item.movie;
                    d.push({
                        title: movie.title,
            ...
            } else if (movie_ == '11' || movie_ == '21' || movie_ == '24' || movie_ == '18' || movie_ == '22' || movie_ == '23' || movie_ == '19') {
                if (MY_PAGE == 1) {
                    d.push({
                        title: '精选分类',
                        desc: getMyVar('cmmovie_flod', '0') == '0' ? '点击展开' : '点击折叠',
                        img: 'https://igtcqqindd4.icu/h5/static/img-manhua/mhhot.png',
                        url: $('#noLoading#').lazyRule(() => {
                            let d = [];
                            putMyVar('cmmovie_flod', getMyVar('cmmovie_flod', '0') == '0' ? '1' : '0');
                            let movie_ = getMyVar('movie', getMyVar('movie_index'));
                            ...
                            if (getMyVar('cmmovie_flod', '0') == '0') {
                                ...
                                let cmlist_ = getMyVar('cmlist_' + movie_, getMyVar('cmlist_index_' + movie_));
                                cmlist.slice(5).forEach((data, index) => {
                                    d.push({
                                        title: cmlist_ == data.id ? $.require('csdown').strong(data.name, 'ff6699') : data.name,
                                        img: $.require('csdown').images(data.imgurl),
                                        url: $('#noLoading#').lazyRule((n, name, id) => {
                                            putMyVar(n, id);
                                            return $.require('csdown').movie_refresh();
                                        }, 'cmlist_' + movie_, data.name, data.id),
                                        col_type: 'icon_5',
                                        extra: {
                                            id: 'cmlist_' + (index + 5),
                                            cls: 'cm_3',
                    ...
                    if (!storage0.getMyVar('movie_cmlist_' + movie_)) {
                        let movie_list = this.post('movie/fengleidataShow/' + movie_ + '/' + pg + '/15/' + getMyVar('movie_cate', 'zx')).cmList;
                    ...
                    cmlist.forEach((data, index) => {
                        d.push({
                            title: cmlist_ == data.id ? this.strong(data.name, 'ff6699') : data.name,
                            img: this.images(data.imgurl),
                            url: $('#noLoading#').lazyRule((n, name, id) => {
                                putMyVar(n, id);
                                return $.require('csdown').movie_refresh();
                ...
                let cmlist_ = getMyVar('cmlist_' + movie_, getMyVar('cmlist_index_' + movie_));
                let cmdata = this.post('movie/cmdata/' + cmlist_ + '/' + pg + '/15/' + getMyVar('movie_cate', 'zx')).list;
                cmdata.forEach(item => {
                    let movie = item.movie;
                    d.push({
                        title: movie.title,
                ...
                let movie_list = this.post('movie/fengleidataShow/' + movie_ + '/' + pg + '/15/' + getMyVar('movie_cate', 'zx'));
                ...
                if (getMyVar('movie_flod', '0') == '0') {
                    storage0.getMyVar('movie_userList_' + movie_).slice(0, 4).forEach(data => {
                        d.push(data)
                    })
                } else {
    ...
    movie_more: function() {
        ...
        try {
            ...
            let list = this.post('movie/moredata/' + id + '/' + pg + '/15/' + getMyVar('user_cate', 'time')).list;
            list.forEach(item => {
                let movie_ = item.movie;
                d.push({
                    title: movie_.title,
    ...
    movie_user: function() {
        ...
        try {
            if (MY_PAGE == 1) {
                ...
                d.push({
                    title: user.name,
                    desc: '作品：' + user.zpnum + ' 粉丝：' + user.fsnumV + ' 观看：' + user.looknum,
            ...
            let list = this.post('movieUser/moredata/' + id + '/' + pg + '/15/' + getMyVar('user_cate', 'time')).list;
            list.forEach(item => {
                let movie_ = item.movie;
                d.push({
                    title: movie_.title,
    ...
    movie_refresh: function() {
        ...
        let movie_ = getMyVar('movie', getMyVar('movie_index'));
        let cmlist_ = getMyVar('cmlist_' + movie_, getMyVar('cmlist_index_' + movie_));
        let cmdata = this.post('movie/cmdata/' + cmlist_ + '/1/15/' + getMyVar('movie_cate', 'zx')).list;
        cmdata.forEach(item => {
            let movie = item.movie;
            d.push({
                title: movie.title,
    ...
    pic: function() {
        ...
        try {
            if (MY_PAGE == 1) {
                ...
                fenlei.forEach(data => {
                    d.push({
                        title: pic_ == data.id ? this.strong(data.name, 'ff6699') : data.name,
                        url: $('#noLoading#').lazyRule((n, name, id) => {
                            putMyVar(n, id);
                            refreshPage(false);
                            return 'hiker://empty';
                        }, 'pic', data.name, data.id),
                        col_type: 'scroll_button',
                        extra: {
            ...
            let pic_ = getMyVar('pic', getMyVar('pic_index'));
            let list = this.post('book/st/mhmoredata/' + pic_ + '/' + pg + '/15/' + getMyVar('pic_', 'time'));
            list.records.forEach(data => {
                let pic_arr = list.zhangjies[data.id + ''][0].imgurl.split(',').map(data => this.images(data));
                d.push({
                    title: data.title,
                    desc: '共' + pic_arr.length + '张图',
                    img: this.images(data.imgurl.includes('.html') ? data.imgurl + '#.jpg#' : data.imgurl),
    ...
    novel: function() {
        ...
        try {
            if (MY_PAGE == 1) {
                ...
                fenlei.forEach(data => {
                    d.push({
                        title: novel_ == data.id ? this.strong(data.name, 'ff6699') : data.name,
                        url: $('#noLoading#').lazyRule((n, name, id) => {
                            putMyVar(n, id);
                            refreshPage(false);
                            return 'hiker://empty';
                        }, 'novel', data.name, data.id),
                        col_type: 'scroll_button',
                        extra: {
            ...
            let novel_ = getMyVar('novel', getMyVar('novel_index'));
            let list = this.post('book/md/xsmoredata/' + novel_ + '/' + pg + '/15/' + getMyVar('novel_', 'time'));
            list.records.forEach(data => {
                d.push({
                    title: data.title,
                    desc: '共' + data.zhangjieNum + '章',
    ...
    search: function() {
        ...
        try {
            ...
            let list = this.post(分类 + '/search/' + pg + '/15/' + getMyVar('search_cate', 'time'), JSON.stringify({
